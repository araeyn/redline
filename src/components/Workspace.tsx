        ...
        <>
          <div className="flex-1 relative flex items-center justify-center p-12 overflow-visible">
            ...
            >
              <div className="relative inline-block overflow-visible border border-zinc-100 shadow-inner p-1">
                ...
                {/* Actual Relight simulation using mix-blend modes */}
                <AnimatePresence>
                  {tab === "relight" && isRelit && (
                    <>
                      <motion.div
                        ...
                        style={{
                          background: `radial-gradient(circle at ${50 + (sunPos.x / 5)}% ${50 + (sunPos.y / 5)}%, #fff 0%, #000 80%)`,
                        }}
                      />
                      <motion.div
                        initial={{ opacity: 0 }}
                        ...
                        style={{
                          background: `radial-gradient(circle at ${50 + (sunPos.x / 5)}% ${50 + (sunPos.y / 5)}%, #fff 0%, transparent 40%)`,
                        }}
                      />
                    </>
                  )}
                ...
                <AnimatePresence>
                  {tab === "layers" && (
                    ...
                    >
                       <div className="absolute inset-0 layer-edge-detect opacity-60" />
                       ...
                       <div className="absolute inset-0 layer-value-map bg-black opacity-40" />
                       <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 border-[3px] border-red-500/10 rounded-full scale-110 opacity-50 blur-[2px]" />
                    </motion.div>
                  )}
                  {tab === "materials" && (
                    <motion.div
              ...
              {/* Sun Handle - No drift, precise dragging */}
              <AnimatePresence>
                {tab === "relight" && (
                  <motion.div
          ...
          >
              ...
              {tab === "critique" ? (
                  ...
                  {displayedCritiques.length > 0 && (
                    <motion.div {...blurFade} className="space-y-8">
                      <div className="p-8 border border-zinc-100 bg-zinc-50/50 rounded-2xl text-center">
                        <p className="text-zinc-600 text-lg font-serif italic italic text-center">Review the markings on the canvas.</p>
                      </div>
                      ...
                      <button onClick={() => {setStatus("idle"); setDisplayedCritiques([])}} className="w-full py-4 border border-zinc-200 text-zinc-400 font-bold rounded-xl transition-all text-xs uppercase tracking-widest font-outfit">Clear Markings</button>
                    </motion.div>
                  )}
                </div>
              ) : tab === "relight" ? (
                <div className="space-y-10">
                  <div className="space-y-3">
                    ...
                    <p className="text-zinc-500 font-serif text-xl leading-snug italic italic">Calculate volumetric depth through shadow projection.</p>
                  </div>
                  <div className="p-8 border border-zinc-100 bg-zinc-50/50 rounded-2xl text-center">
            ...
            <div className="p-10 bg-zinc-50/30 border-t border-zinc-100 relative z-10 flex flex-col items-center gap-2">
               ...
               <span className="text-[8px] font-bold text-zinc-300 tracking-widest uppercase font-outfit">System 1.3.0-PREMIUM</span>
            </div>
          </motion.div>
        </>
      )}
    ...
    >
      <div className="relative flex items-center justify-center cursor-pointer group">
        <div className="w-6 h-6 flex items-center justify-center">
          ...
          {/* High contrast pretty dot: white circle with red border */}
